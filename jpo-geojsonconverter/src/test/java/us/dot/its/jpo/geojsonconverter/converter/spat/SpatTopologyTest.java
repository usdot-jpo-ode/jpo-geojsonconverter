...
public class SpatTopologyTest {
    ...
    String kafkaTopicSpatGeoJson = "topic.SpatGeoJson";
    ...
    public void testTopology() {
        Topology topology = SpatTopology.build(kafkaTopicOdeSpatJson, kafkaTopicSpatGeoJson, kafkaTopicMapGeoJson, spatJsonValidator);
        try (TopologyTestDriver driver = new TopologyTestDriver(topology)) {
            TestInputTopic<String, String> inputMapGeoJsonTopic = driver.createInputTopic(
            ...
            TestInputTopic<Void, String> inputOdeSpatJsonTopic = driver.createInputTopic(
                ...
                Serdes.String().serializer());
            TestOutputTopic<String, SpatFeatureCollection> outputTopic = driver.createOutputTopic(
                kafkaTopicSpatGeoJson, 
                Serdes.String().deserializer(), 
                JsonSerdes.SpatGeoJson().deserializer());
            
            // Send serialized MapGeoJson to MapGeoJson topic
            inputMapGeoJsonTopic.pipeInput(new TestRecord<String, String>("172.19.0.1:12110", mapGeoJsonString));
            ...
            List<KeyValue<String, SpatFeatureCollection>> spatGeoJsonResults = outputTopic.readKeyValuesToList();
            assertEquals(spatGeoJsonResults.size(), 1);

            KeyValue<String, SpatFeatureCollection> spatGeoJson = spatGeoJsonResults.get(0);
            assertNotNull(spatGeoJson.key);
            assertEquals("172.19.0.1:12110", spatGeoJson.key);
            assertNotNull(spatGeoJson.value);
            assertEquals(2, spatGeoJson.value.getFeatures().length);
            assertEquals(-105.09115232580189, spatGeoJson.value.getFeatures()[0].getGeometry().getCoordinates()[0]);
