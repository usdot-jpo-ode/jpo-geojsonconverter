...
public class ProcessedMapGeoJsonDeserializer implements Deserializer<ProcessedMap<LineString>> {
    ...
    @Override
    public ProcessedMap<LineString> deserialize(String topic, byte[] data) {
        if (data == null) {
                return null;
        }
        try {
            JavaType javaType = mapper.getTypeFactory().constructParametricType(ProcessedMap.class, LineString.class);
            return (ProcessedMap<LineString>) mapper.readValue(data, javaType);
        } catch (IOException e) {
